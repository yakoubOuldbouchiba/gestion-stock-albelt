...
public interface ArticleRepository extends JpaRepository<Article, UUID> {
    ...
    @org.springframework.data.jpa.repository.EntityGraph(attributePaths = {"color"})
    ...
    @Query(value = """
        ...
        where (:search is null or :search = '' or lower(a.reference) like concat('%', lower(:search), '%') or lower(a.name) like concat('%', lower(:search), '%') or lower(a.code) like concat('%', lower(:search), '%'))
        """)
    org.springframework.data.domain.Page<Article> findFiltered(@Param("search") String search, org.springframework.data.domain.Pageable pageable);
}
