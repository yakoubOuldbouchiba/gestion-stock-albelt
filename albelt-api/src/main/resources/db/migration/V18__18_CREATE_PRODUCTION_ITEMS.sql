...
CREATE TABLE IF NOT EXISTS production_items (
    ...
    commande_item_id UUID NOT NULL REFERENCES commande_items(id) ON DELETE CASCADE,

    -- Source Material (exactly one)
    roll_id UUID REFERENCES rolls(id) ON DELETE RESTRICT,
    waste_piece_id UUID REFERENCES waste_pieces(id) ON DELETE RESTRICT,

    -- Dimensions (per piece)
    piece_length_m DECIMAL(10,2) NOT NULL CHECK (piece_length_m > 0),
    piece_width_mm INTEGER NOT NULL CHECK (piece_width_mm > 0),

    -- Quantity
    quantity INTEGER NOT NULL DEFAULT 1 CHECK (quantity > 0),

    -- Area (calculated)
    area_per_piece_m2 DECIMAL(12,4) NOT NULL CHECK (area_per_piece_m2 > 0),
    total_area_m2 DECIMAL(12,4) NOT NULL CHECK (total_area_m2 > 0),
...
-- ============================================================================
CREATE OR REPLACE FUNCTION calculate_production_item_area()
RETURNS TRIGGER AS $$
BEGIN
    NEW.area_per_piece_m2 := (NEW.piece_width_mm::DECIMAL / 1000.0) * NEW.piece_length_m;
    NEW.total_area_m2 := NEW.area_per_piece_m2 * NEW.quantity;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_production_items_calculate_area') THEN
...
-- ============================================================================
CREATE OR REPLACE FUNCTION enforce_production_item_area()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    IF NEW.roll_id IS NOT NULL THEN
        SELECT area_m2 INTO source_area
        FROM rolls
        WHERE id = NEW.roll_id;

        SELECT COALESCE(SUM(total_area_m2), 0) INTO sibling_sum
    ...
    IF source_area IS NOT NULL AND (sibling_sum + NEW.total_area_m2) > source_area THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_production_items_validate_area') THEN
...
-- ============================================================================
CREATE OR REPLACE FUNCTION enforce_production_item_quantity()
RETURNS TRIGGER AS $$
DECLARE
    item_quantity INTEGER;
    sibling_sum INTEGER;
BEGIN
    SELECT quantite INTO item_quantity
    FROM commande_items
    WHERE id = NEW.commande_item_id;

    SELECT COALESCE(SUM(quantity), 0) INTO sibling_sum
    FROM production_items
    WHERE commande_item_id = NEW.commande_item_id
      AND (NEW.id IS NULL OR id <> NEW.id);

    IF item_quantity IS NOT NULL AND (sibling_sum + NEW.quantity) > item_quantity THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_production_items_validate_quantity') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_production_items_updated_at') THEN
