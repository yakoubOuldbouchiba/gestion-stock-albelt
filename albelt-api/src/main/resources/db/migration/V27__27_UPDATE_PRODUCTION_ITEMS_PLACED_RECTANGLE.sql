...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_production_items_validate_area') THEN
        DROP TRIGGER trigger_production_items_validate_area ON production_items;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_production_items_validate_quantity') THEN
...
ALTER TABLE production_items
    DROP CONSTRAINT IF EXISTS production_items_single_source;
...
ALTER TABLE production_items
    DROP COLUMN IF EXISTS commande_item_id,
    DROP COLUMN IF EXISTS roll_id,
    DROP COLUMN IF EXISTS waste_piece_id;
...
BEGIN
    ...
    IF rect_area IS NOT NULL AND (sibling_sum + NEW.total_area_m2) > rect_area THEN
...
DECLARE
    ...
    sibling_sum INTEGER;
BEGIN
    SELECT commande_item_id INTO item_id
    FROM placed_rectangles
    WHERE id = NEW.placed_rectangle_id;
    ...
    SELECT COALESCE(SUM(pi.quantity), 0) INTO sibling_sum
...
-- ============================================================================
CREATE OR REPLACE FUNCTION enforce_production_item_dimensions()
RETURNS TRIGGER AS $$
DECLARE
    rect_width INTEGER;
    rect_height INTEGER;
...
BEGIN
    SELECT width_mm, height_mm INTO rect_width, rect_height
    FROM placed_rectangles
    WHERE id = NEW.placed_rectangle_id;

    IF rect_width IS NULL OR rect_height IS NULL THEN
        RETURN NEW;
    END IF;

    length_mm := NEW.piece_length_m * 1000;

    IF NEW.piece_width_mm > rect_width THEN
        RAISE EXCEPTION 'Piece width (% mm) exceeds placed rectangle width (% mm)',
                       NEW.piece_width_mm, rect_width;
    END IF;

    IF length_mm > rect_height THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_production_items_validate_dimensions') THEN
