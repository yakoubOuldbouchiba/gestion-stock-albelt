...
CREATE TABLE IF NOT EXISTS user_altier (
    ...
    altier_id UUID NOT NULL REFERENCES altier(id) ON DELETE CASCADE,
    
    -- Unique constraint
    UNIQUE(user_id, altier_id),
...
CREATE TABLE IF NOT EXISTS transfer_bons (
    ...
    -- Transfer Locations
    from_altier_id UUID NOT NULL REFERENCES altier(id) ON DELETE RESTRICT,
    to_altier_id UUID NOT NULL REFERENCES altier(id) ON DELETE RESTRICT,
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE roll_movements ADD COLUMN transfer_bon_id UUID;
    END IF;
END $$;
