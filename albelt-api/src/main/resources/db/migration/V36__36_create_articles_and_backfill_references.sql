...
CREATE TABLE IF NOT EXISTS articles (
    ...
    nb_plis INTEGER NOT NULL CHECK (nb_plis > 0),
...
ALTER TABLE rolls ADD COLUMN IF NOT EXISTS article_id UUID;
ALTER TABLE waste_pieces ADD COLUMN IF NOT EXISTS article_id UUID;
ALTER TABLE commande_items ADD COLUMN IF NOT EXISTS article_id UUID;
ALTER TABLE purchase_bon_items ADD COLUMN IF NOT EXISTS article_id UUID;
...
ON CONFLICT (material_type, thickness_mm, nb_plis, reference) DO NOTHING;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_rolls_article') THEN
        ALTER TABLE rolls
            ADD CONSTRAINT fk_rolls_article
            FOREIGN KEY (article_id) REFERENCES articles(id) ON DELETE RESTRICT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_waste_pieces_article') THEN
        ALTER TABLE waste_pieces
            ADD CONSTRAINT fk_waste_pieces_article
            FOREIGN KEY (article_id) REFERENCES articles(id) ON DELETE RESTRICT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_commande_items_article') THEN
        ALTER TABLE commande_items
            ADD CONSTRAINT fk_commande_items_article
            FOREIGN KEY (article_id) REFERENCES articles(id) ON DELETE RESTRICT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_purchase_bon_items_article') THEN
