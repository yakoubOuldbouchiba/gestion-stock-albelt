...
(
    ...
    parent_waste_piece_id UUID REFERENCES waste_pieces
(
    id
)
  ON DELETE SET NULL,
...
ALTER TABLE rolls
DROP
COLUMN IF EXISTS original_quantity,
    ADD COLUMN color_id UUID REFERENCES colors(id) ON DELETE
SET NULL;
...
-- ============================================================================
CREATE
OR REPLACE FUNCTION validate_waste_piece_dimensions()
RETURNS TRIGGER AS $$
DECLARE
source_roll RECORD;
    source_piece
...
BEGIN
    IF
NEW.parent_waste_piece_id IS NOT NULL THEN
SELECT width_mm, length_m, area_m2, color_id
INTO source_piece
FROM waste_pieces
...
END IF;
        ...
        IF
NEW.length_m > source_piece.length_m THEN
...
END IF;
        ...
        IF
NEW.area_m2 > source_piece.area_m2 THEN
...
END IF;
        ...
        IF
NEW.length_m > source_roll.length_m THEN
...
END IF;
        ...
        IF
NEW.area_m2 > source_roll.area_m2 THEN
...
NEW.color_id IS NULL THEN
        ...
        ELSIF
NEW.color_id <> source_roll.color_id THEN
...
-- ============================================================================
CREATE
OR REPLACE FUNCTION enforce_waste_piece_child_area()
RETURNS TRIGGER AS $$
DECLARE
...
END IF;
    ...
    IF
parent_area IS NOT NULL AND (sibling_sum + NEW.area_m2) > parent_area THEN
...
-- ============================================================================
CREATE
OR REPLACE FUNCTION update_roll_waste_area()
RETURNS TRIGGER AS $$
BEGIN
    IF
TG_OP = 'INSERT' THEN
