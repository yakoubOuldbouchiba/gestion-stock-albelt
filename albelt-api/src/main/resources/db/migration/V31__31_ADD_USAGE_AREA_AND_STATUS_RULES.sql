...
CREATE
OR REPLACE FUNCTION sync_roll_usage_fields()
RETURNS TRIGGER AS $$
BEGIN
    NEW.used_area_m2
:= COALESCE(NEW.used_area_m2, 0);
    NEW.available_area_m2
:= GREATEST(COALESCE(NEW.area_m2, 0) - NEW.used_area_m2, 0);
RETURN NEW;
END;
$$
LANGUAGE plpgsql;
...
CREATE
OR REPLACE FUNCTION sync_waste_usage_fields()
RETURNS TRIGGER AS $$
BEGIN
    NEW.used_area_m2
:= COALESCE(NEW.used_area_m2, 0);
...
IF
v_status <> 'ARCHIVED' THEN
SELECT COUNT(*)
INTO v_placement_count
FROM placed_rectangles
WHERE roll_id = p_roll_id;
v_next_status
:= CASE WHEN v_placement_count > 0 THEN 'OPENED' ELSE 'AVAILABLE'
END;
...
v_threshold_area
:= (COALESCE(v_min_width_mm, 0)::DECIMAL / 1000.0) * COALESCE(v_min_length_m, 0);

        IF
v_available_area <= v_threshold_area THEN
...
CREATE
OR REPLACE FUNCTION recalc_waste_usage_and_status(p_waste_id UUID)
RETURNS VOID AS $$
DECLARE
v_used_area DECIMAL(12,4);
    v_area
...
INTO v_placement_count
FROM placed_rectangles
WHERE waste_piece_id = p_waste_id;
v_next_status
:= CASE WHEN v_placement_count > 0 THEN 'OPENED' ELSE 'AVAILABLE'
END;
...
CREATE
OR REPLACE FUNCTION update_source_usage_from_placements()
RETURNS TRIGGER AS $$
BEGIN
    IF
TG_OP = 'INSERT' THEN
        ...
        PERFORM
recalc_waste_usage_and_status(NEW.waste_piece_id);
    ELSIF
TG_OP = 'UPDATE' THEN
        PERFORM recalc_roll_usage_and_status(NEW.roll_id);
        PERFORM
recalc_waste_usage_and_status(NEW.waste_piece_id);
        IF
NEW.roll_id IS DISTINCT FROM OLD.roll_id THEN
            PERFORM recalc_roll_usage_and_status(OLD.roll_id);
END IF;
        IF
NEW.waste_piece_id IS DISTINCT FROM OLD.waste_piece_id THEN
...
TG_OP = 'DELETE' THEN
        PERFORM recalc_roll_usage_and_status(OLD.roll_id);
        PERFORM
recalc_waste_usage_and_status(OLD.waste_piece_id);
END IF;
RETURN NULL;
END;
$$
...
UPDATE rolls r
SET used_area_m2      = COALESCE((SELECT SUM((pr.width_mm::DECIMAL * pr.height_mm::DECIMAL) / 1000000.0)
                                  FROM placed_rectangles pr
                                  WHERE pr.roll_id = r.id), 0),
    available_area_m2 = GREATEST(
            r.area_m2 - COALESCE((SELECT SUM((pr.width_mm::DECIMAL * pr.height_mm::DECIMAL) / 1000000.0)
                                  FROM placed_rectangles pr
                                  WHERE pr.roll_id = r.id), 0), 0)
WHERE r.id IS NOT NULL;

UPDATE waste_pieces w
SET used_area_m2      = COALESCE((SELECT SUM((pr.width_mm::DECIMAL * pr.height_mm::DECIMAL) / 1000000.0)
                                  FROM placed_rectangles pr
                                  WHERE pr.waste_piece_id = w.id), 0),
    available_area_m2 = GREATEST(
            w.area_m2 - COALESCE((SELECT SUM((pr.width_mm::DECIMAL * pr.height_mm::DECIMAL) / 1000000.0)
                                  FROM placed_rectangles pr
                                  WHERE pr.waste_piece_id = w.id), 0), 0)
...
FOR r IN
SELECT id
FROM rolls LOOP PERFORM recalc_roll_usage_and_status(r.id);
END LOOP;
FOR r IN
SELECT id
FROM waste_pieces LOOP PERFORM recalc_waste_usage_and_status(r.id);
END LOOP;
END $$;
