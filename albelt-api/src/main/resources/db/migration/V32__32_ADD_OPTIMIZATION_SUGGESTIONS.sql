...
CREATE TABLE IF NOT EXISTS optimization_plans (
    ...
    used_area_m2 DECIMAL(12,4) NOT NULL DEFAULT 0,
    waste_area_m2 DECIMAL(12,4) NOT NULL DEFAULT 0,
    utilization_pct DECIMAL(6,2) NOT NULL DEFAULT 0,
    svg TEXT,
...
CREATE TABLE IF NOT EXISTS optimization_placements (
    ...
    waste_piece_id UUID REFERENCES waste_pieces(id),
    ...
    piece_length_m DECIMAL(10,2) NOT NULL,
    area_m2 DECIMAL(12,4) NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT optimization_placements_source_check CHECK (
        (source_type = 'ROLL' AND roll_id IS NOT NULL AND waste_piece_id IS NULL)
        OR (source_type = 'WASTE' AND waste_piece_id IS NOT NULL AND roll_id IS NULL)
    )
);
