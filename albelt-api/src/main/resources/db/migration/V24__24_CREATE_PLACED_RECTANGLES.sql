...
CREATE TABLE IF NOT EXISTS placed_rectangles (
    ...
    waste_piece_id UUID REFERENCES waste_pieces(id) ON DELETE CASCADE,
...
-- ============================================================================
CREATE OR REPLACE FUNCTION validate_placed_rectangle_bounds()
RETURNS TRIGGER AS $$
DECLARE
    source_width_mm INTEGER;
    source_length_m DECIMAL(10,2);
    max_length_mm DECIMAL(12,4);
BEGIN
    IF NEW.roll_id IS NOT NULL THEN
        SELECT width_mm, length_m INTO source_width_mm, source_length_m
        FROM rolls
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_placed_rectangle_bounds') THEN
        CREATE TRIGGER trigger_placed_rectangle_bounds
        BEFORE INSERT OR UPDATE ON placed_rectangles
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_placed_rectangles_updated_at') THEN
        CREATE TRIGGER trigger_placed_rectangles_updated_at
        BEFORE UPDATE ON placed_rectangles
