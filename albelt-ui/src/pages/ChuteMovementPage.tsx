...
export function ChuteMovementPage() {
  ...
  const source = useMemo(() => (wasteId ? ({ type: 'WASTE_PIECE' as const, id: wasteId }) : null), [wasteId]);
  const history = useMovementHistory({ source, t });
  const isBusy = isLocked('chute-move-confirm') || isLocked('chute-move-create');

  useEffect(() => {
    if (!wasteId) return;

    const loadWaste = async () => {
      const res = await WastePieceService.getById(wasteId);
    ...
    <div className="page-container movement-page">
      ...
      {showMovementForm && user && wasteId && (
        <RollMovementForm
          ...
          userId={user.id}
          fixedSource={{ sourceType: 'WASTE_PIECE', wastePieceId: wasteId, fromAltierID: fromAltierID || undefined }}
        />
      )}
